Song1Header:
    ...
    .byte STREAM_MUSIC_SQ1     ;which stream
    ;.byte $00
    .byte $01           ;status byte (stream enabled)
    .byte CHANNEL_SQUARE_1      ;which channel
    .byte $70           ;initial duty (01)
    .byte ve_tgl_1      ;volume envelope
    .word song1_square1 ;pointer to stream
    .byte $53           ;tempo
    
    .byte STREAM_MUSIC_SQ2     ;which stream
    ;.byte $00
    .byte $01           ;status byte (stream enabled)
    .byte CHANNEL_SQUARE_2      ;which channel
    .byte $B0           ;initial duty (10)
    .byte ve_tgl_2      ;volume envelope
    .word song1_square2 ;pointer to stream
    .byte $53           ;tempo
    
    .byte STREAM_MUSIC_TRI     ;which stream
    ;.byte $00
    .byte $01           ;status byte (stream enabled)
    .byte CHANNEL_TRIANGLE      ;which channel
    .byte $80           ;initial volume (on)
    .byte ve_tgl_2      ;volume envelope
    .word song1_tri     ;pointer to stream
    .byte $53           ;tempo
    
    .byte STREAM_MUSIC_NOI     ;which stream
    ;.byte $00
    .byte $01           ;enabled
    .byte CHANNEL_NOISE     
...
song1_square2:
    .byte sixteenth
    .byte NOTE_REST    ;offset for delay effect
    .byte eighth
.loop_point:
    .byte NOTE_REST
    ...
    .byte E4, A4, Cs5, E5, A5, E5, sixteenth, Cs5, NOTE_REST
    .byte eighth
